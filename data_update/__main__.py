...
def get_photos() -> set[str]:
    ...
    p.download_album(
        "Public Noodle",
        "./web/data/photos/thumbnails/",
        200,
        200,
        exclude=existing_photos,
    )
    p.download_album(
        "Public Noodle",
        "./web/data/photos/full_res/",
        1500,
        1500,
        exclude=existing_photos,
    )
    ...
    return existing_photos | {
        os.path.basename(p) for p in glob("./web/data/photos/full_res/*")
    }


def create_json_data(
    shed_ranges: pd.DataFrame,
    feeding_df: pd.DataFrame,
    weight_df: pd.DataFrame,
    photo_fnames: set[str],
    output_path: str,
) -> None:
    os.makedirs(os.path.split(output_path)[0], exist_ok=True)

    images = [
        {"fname": fname, "date": pd.to_datetime(match.group(1)).strftime("%Y-%m-%d")}
    ...
    data = {
        "shedding": [
            [r.min.strftime("%Y-%m-%d"), r.max.strftime("%Y-%m-%d")]
            for r in shed_ranges.itertuples()
        ],
        "feeding_accepted": {
            "date": feeding_df.query("accepted")["date"]
            .dt.strftime("%Y-%m-%d")
            .to_list(),
...
def plot_static_graph(
    ...
    shed_plot_xs: list[pd.Timestamp] = sum(
        (
            [r.min, r.max, r.max + pd.Timedelta(hours=1)]
            for r in shed_ranges.itertuples()
        ),
        start=[],
    )
    shed_plot_ys = np.array(
        sum(([1, 1, np.nan] for _ in range(len(shed_ranges))), start=[])
    )
    ...
    plt.xticks(
        pd.date_range(
            weight_df["date"].min() - pd.Timedelta(weeks=4),
            weight_df["date"].max() + pd.Timedelta(weeks=4),
            freq="MS",
        )
    )
    ...
    with pysftp.Connection(
        ...
        with sftp.cd("public_html"):
            put_r_portable(
                ...
                "web",
                "noodle",
                skip_if_exists=(lambda p: p[-4:] in (".jpg", ".gif")),
            )
...
def main():
    ...
    create_json_data(
        **tab_data,
        photo_fnames=all_photo_fnames,
        output_path="./web/data/noodle_data.json",
    )
